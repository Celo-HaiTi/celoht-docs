...
Before touching code, every developer contributor is asked to read [MISSION.md](./MISSION.md) *(Brand repository)* and [NO_TOKEN_POLICY.md](./NO_TOKEN_POLICY.md) — understanding *why* CeloHT is built the way it is (non-custodial, minimal on-chain data, no token) prevents well-intentioned contributions that accidentally work against these principles.
